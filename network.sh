...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Staff Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-staff.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating Account Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-accountant.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/staff/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating Accountant Identities"
  ...
  infoln "Generating CCP files for Staff and Accountant"
...
# call the script to create the channel, join the peers of staff and accountant,
...
function networkDown() {
  # stop org3 containers also in addition to staff and accountant, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/staff/msp organizations/fabric-ca/staff/tls-cert.pem organizations/fabric-ca/staff/ca-cert.pem organizations/fabric-ca/staff/IssuerPublicKey organizations/fabric-ca/staff/IssuerRevocationPublicKey organizations/fabric-ca/staff/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/accountant/msp organizations/fabric-ca/accountant/tls-cert.pem organizations/fabric-ca/accountant/ca-cert.pem organizations/fabric-ca/accountant/IssuerPublicKey organizations/fabric-ca/accountant/IssuerRevocationPublicKey organizations/fabric-ca/accountant/fabric-ca-server.db'
